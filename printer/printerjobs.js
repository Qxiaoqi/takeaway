...
/**
 * 打印菜单
 * @param  {Array} dishArr  文字内容
 */
printerJobs.prototype.printDishArr = function (dishArr) {
  for (let i = 0; i < dishArr.length; i++) {
    this.text(dishArr[i]+" * 1");
    this._enqueue(commands.LF);
  }
  return this;
};
...
/**
 * 打印二维码
 * @param  {string} content 二维码数据（二值化后的字节数组）  
 */
printerJobs.prototype.drawQR = function (content) {
  if (content) {
    let cmd = [].concat(commands.TEXT_FORMAT.TXT_ALIGN_CT, commands.RASTERBITMAP.QR, content, commands.RASTERBITMAP.PRINT_3);
    this._enqueue(cmd);
  }
  // this.setAlign('ct');
  // this._enqueue(commands.RASTERBITMAP.QR);
  // this.text(content);
  // this._enqueue(commands.RASTERBITMAP.PRINT_3);  
